...
struct AdventureDetailView: View {
    ...
    private func loadAdventure(slug: String) {
        aem.getProductBySlug( slug: slug, completion: { (adventure) in
    ...
    var body: some View {
        ScrollView {
            if (!adventure.isEmpty()) {
                ...
                VStack(alignment: .leading) {
                    ...
                    Text(adventure.productTitle).font(.title)

                    
                    Divider()
                    Spacer()

                    Text("About \(adventure.productTitle)").font(.title2)
                    
                    Spacer()
                    
                    Text(adventure.productDescription)
        ...
        .navigationTitle(adventure.productTitle)
...
struct AdventureDetailView_Previews: PreviewProvider {
    static var previews: some View {
        AdventureDetailView(adventure: TestAdventureBySlug.get(), slug: "fan")
    }
}
