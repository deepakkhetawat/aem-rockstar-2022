...
class Aem: ObservableObject {
    ...
    /// # Products()
    /// Returns all Products using the `mec/all-products-header-footer` persisted query.
    /// For this func call to work, the `mec/all-products-header-footer` query must be deployed to the AEM environment/service specified by the host
    func getAdventures(completion: @escaping ([Adventure]) ->  ()) {
               
        let request = makeRequest(persistedQueryName: "mec/ios-all-products-header-footer-by-locale", params: [ "locale": "en" ])
        ...
        URLSession.shared.dataTask(with: request) { (data, response, error) in
            ...
            if (!data!.isEmpty) {
                ...
                DispatchQueue.main.async {
                    completion(adventures.data.productList.items)
                }
            }
        }.resume();
    }
    ...
    func getProductBySlug(slug: String, completion: @escaping (Adventure) ->  ()) {
        ...
        URLSession.shared.dataTask(with: request) { (data, response, error) in
            ...
            if (!data!.isEmpty) {
                let adventures = try! JSONDecoder().decode(Adventures.self, from: data!)
                DispatchQueue.main.async {
                    if (!adventures.data.productList.items.isEmpty) {
                        completion(adventures.data.productList.items[0])
                    }
                }
            }
        }.resume();
